--- 
title: "Linked List Questions"
description: "Questions on Linked List data structure"
pubDate: 2026-06-17
---
# Linked List Questions

**Singly Linked List**

- [Reverse Linked List (206)](https://leetcode.com/problems/reverse-linked-list/) – most frequent
- [Merge Two Sorted Lists (21)](https://leetcode.com/problems/merge-two-sorted-lists/) - eazy
- [Remove Nth Node From End of List (19)](https://leetcode.com/problems/remove-nth-node-from-end-of-list/)
- [Linked List Cycle (141)](https://leetcode.com/problems/linked-list-cycle/)
- [Linked List Cycle II (142)](https://leetcode.com/problems/linked-list-cycle-ii/)
- [Middle of the Linked List (876)](https://leetcode.com/problems/middle-of-the-linked-list/)
- [Palindrome Linked List (234)](https://leetcode.com/problems/palindrome-linked-list/)
- [Remove Duplicates from Sorted List (83)](https://leetcode.com/problems/remove-duplicates-from-sorted-list/)
- [Intersection of Two Linked Lists (160)](https://leetcode.com/problems/intersection-of-two-linked-lists/)
- [Add Two Numbers (2)](https://leetcode.com/problems/add-two-numbers/)
- [Odd Even Linked List (328)](https://leetcode.com/problems/odd-even-linked-list/)
- [Swap Nodes in Pairs (24)](https://leetcode.com/problems/swap-nodes-in-pairs/)
- [Rotate List (61)](https://leetcode.com/problems/rotate-list/)
- [Partition List (86)](https://leetcode.com/problems/partition-list/)
- [Sort List (148)](https://leetcode.com/problems/sort-list/)
- [Reorder List (143)](https://leetcode.com/problems/reorder-list/)
- [Remove Linked List Elements (203)](https://leetcode.com/problems/remove-linked-list-elements/)

**Doubly Linked List**

- [Design Browser History (1472)](https://leetcode.com/problems/design-browser-history/)
- [Flatten a Multilevel Doubly Linked List (430)](https://leetcode.com/problems/flatten-a-multilevel-doubly-linked-list/)
- ** Design a Doubly Linked List (custom implementation)
- ** [LRU Cache (146)](https://leetcode.com/problems/lru-cache/) – uses DLL + HashMap, very frequent
- [All O`one Data Structure (432)](https://leetcode.com/problems/all-oone-data-structure/)

**Circular Linked List**

- [Insert into a Sorted Circular Linked List (708)](https://leetcode.com/problems/insert-into-a-sorted-circular-linked-list/)
- [Convert Binary Search Tree to Sorted Doubly Linked List (426)](https://leetcode.com/problems/convert-binary-search-tree-to-sorted-doubly-linked-list/)
- [Design Circular Queue (622)](https://leetcode.com/problems/design-circular-queue/)
- [Design Circular Deque (641)](https://leetcode.com/problems/design-circular-deque/)
- [Linked List Cycle (141)](https://leetcode.com/problems/linked-list-cycle/) – detection (Floyd's)

**Advanced / Combination**

- [Merge K Sorted Lists (23)](https://leetcode.com/problems/merge-k-sorted-lists/)
- [Copy List with Random Pointer (138)](https://leetcode.com/problems/copy-list-with-random-pointer/)
- [Reverse Linked List II (92)](https://leetcode.com/problems/reverse-linked-list-ii/) – reverse between positions
- [Reverse Nodes in k-Group (25)](https://leetcode.com/problems/reverse-nodes-in-k-group/)
- [Flatten Binary Tree to Linked List (114)](https://leetcode.com/problems/flatten-binary-tree-to-linked-list/)

**Top "must-do" (interview frequency)**

1. [Reverse Linked List (206)](https://leetcode.com/problems/reverse-linked-list/)
2. [Merge Two Sorted Lists (21)](https://leetcode.com/problems/merge-two-sorted-lists/)
3. [Linked List Cycle (141)](https://leetcode.com/problems/linked-list-cycle/)
4. [Remove Nth Node From End (19)](https://leetcode.com/problems/remove-nth-node-from-end-of-list/)
5. [LRU Cache (146)](https://leetcode.com/problems/lru-cache/)
6. [Copy List with Random Pointer (138)](https://leetcode.com/problems/copy-list-with-random-pointer/)
7. [Merge K Sorted Lists (23)](https://leetcode.com/problems/merge-k-sorted-lists/)
8. [Add Two Numbers (2)](https://leetcode.com/problems/add-two-numbers/)
9. [Palindrome Linked List (234)](https://leetcode.com/problems/palindrome-linked-list/)
10. [Middle of Linked List (876)](https://leetcode.com/problems/middle-of-the-linked-list/)